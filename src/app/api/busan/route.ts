...
import restaurants from "@/data/r.json";
...
export async function GET(request: NextRequest) {
    ...
    console.log(title);
    if (id) {
        const idData = restaurants.find((item) => item.UC_SEQ === Number(id));
        if (idData) {
        ...
        const titleData = restaurants.find((item) => item.MAIN_TITLE === String(title));
        if (titleData) {
...
export async function PUT(request: NextRequest) {
    try {
        ...
        data[index] = { UC_SEQ: numericId, ...updatedData };
